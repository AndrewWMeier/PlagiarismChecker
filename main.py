...
def get_google_results(query, num_results=3):
    try:
        ...
        all_sources = []
        for i, result in enumerate(search_results[:num_results], start=1):  #limit to num_results
            # print(f"Result {i}:\n{result}")
    ...
    for source in sources:
        for sentence_sample in source.get_sentences():
            for sentence_user in user_query:
                ...
                # print("-----------------------------------------------------------------")
                comparison = Comparison(sentence_user, sentence_sample, None, source.get_url())
